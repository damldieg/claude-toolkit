...
| Level | Agent | Use for |
| --- | --- | --- |
| tiny | `ctk-maintenance-tiny` | State updates, narrow checks, short diffs, file discovery, simple documentation. |
| normal | `ctk-feature-normal` | A bounded, self-contained feature, integration, UI change, or focused bug fix. |
| deep | `ctk-systems-deep` | Reproducible difficult bugs, performance issues, cross-cutting systems, or risky refactors. |
| architect | `ctk-architecture-review` | Compare consequential architectural options before committing to one. |
...
A milestone is a user-visible feature slice, a completed bug investigation/fix, a validated system change, a release/commit-ready checkpoint, or a decision that changes future implementation. At a milestone, run `/ctk-checkpoint` (or make its equivalent update) once.
